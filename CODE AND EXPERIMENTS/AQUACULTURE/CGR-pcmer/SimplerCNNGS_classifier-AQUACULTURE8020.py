...
## FUNCTIONS
from AQUACULTURE.functions_Net_AQUACULTURE import create_SimplerCNN, preprocessing, metrics, saveModel, plot_loss_accuracy, saveConfMatrixClassReport
...
if __name__ == '__main__':
      ...
      saveModel(dataset_C0, model_cnn, 'Simpler CNN', type_encoder + 'Simpler CNN' + str(batch_size) + str(epoch) + 'train80-test20')    

      acc = plot_loss_accuracy(history, model_cnn, X_test, y_test, dataset_C0, 'Simpler CNN', type_encoder + 'Simpler CNN' + str(batch_size) + str(epoch) + 'train80-test20')

      conf_matrix, class_report = metrics(X_test, y_test, model_cnn)
      print('\n', conf_matrix, '\n', class_report)

      # save the results of classification model
      saveConfMatrixClassReport('Simpler CNN', training_time, acc, conf_matrix, class_report, dataset_C0, type_encoder + 'Simpler CNN' + str(batch_size) + str(epoch) + 'train80-test20')
