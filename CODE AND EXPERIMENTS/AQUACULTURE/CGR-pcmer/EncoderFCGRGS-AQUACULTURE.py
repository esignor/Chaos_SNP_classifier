...
def img(haplotypes_file, data, mortality_data, out_directory, selected_features, init, cl):
        print('len',len( selected_features))
        for n in range(init, len(selected_features), 2):
            ...
            k = 10
            #directory_png = out_directory + '/FCGR k=' + str(k) + '/' + 'GS 4 points/' +  haplotypes_file + '/' + str(cl) + '/' + dir_mortality
...
def img_setCluster(haplotypes_file, data, mortality_data, out_directory, path_datasets, cl):
        ...
        t3= multiprocessing.Process(target = img, args = (haplotypes_file, data, mortality_data, out_directory, selected_features, 0, cl))
        t4 = multiprocessing.Process(target = img, args = (haplotypes_file, data, mortality_data, out_directory, selected_features, 1, cl))
        t3.start(); t4.start()
...
if __name__ == '__main__':
    ...
    t1 = multiprocessing.Process(target = img_setCluster, args = (haplotypes_file, data, mortality_data, out_directory, path_datasets, 0))
    t2 = multiprocessing.Process(target = img_setCluster, args = (haplotypes_file, data, mortality_data, out_directory, path_datasets, 1))
    t1.start(); t2.start()
